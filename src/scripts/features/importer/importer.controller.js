...
export function hasImportDraftContent() {
	return !draft.result && Boolean(draft.rawText.trim() || draft.fileName || draft.records.length || draft.step > 1);
}
...
export async function finalizeImport() {
	...
	draft.result = {processed: draft.records.length, imported: imported.length, invalid, skipped};
	draft.step = 4;
...
function cancelButton() {
	return '<button class="button button--ghost" type="button" data-action="cancel-import">Cancelar importação</button>';
}
...
function renderStepOne() {
	...
	return `
		...
		<div class="wizard-actions">${cancelButton()}<button class="button button--primary" type="button" data-action="import-next">Continuar</button></div>
	`;
}
...
function renderStepTwo() {
	...
	return `
		...
		<div class="wizard-actions wizard-actions--three">${cancelButton()}<button class="button button--ghost" type="button" data-action="import-back">Voltar</button><button class="button button--primary" type="button" data-action="import-preview">Validar e visualizar</button></div>
	`;
}
...
function tagsMarkup(tags = []) {
	if (!tags.length) return '<span class="text-muted">Sem tags</span>';
	return `<div class="preview-tags">${tags.map((tag) => `<span class="badge">${escapeHTML(tag)}</span>`).join('')}</div>`;
}

function previewRow(record) {
	return `
		<tr class="preview-record preview-record--${record.status}">
			<td data-label="Linha"><strong>${record.lineNumber}</strong></td>
			<td data-label="Frente">${escapeHTML(record.front || '—')}</td>
			<td data-label="Verso">${escapeHTML(record.back || '—')}</td>
			<td data-label="Tags">${tagsMarkup(record.tags)}</td>
			<td data-label="Status">${statusBadge(record)}</td>
			<td data-label="Detalhes"><span class="preview-reason">${escapeHTML(record.message || '—')}</span></td>
		</tr>
	`;
}

function renderStepThree() {
	const visibleRecords = draft.records.slice(0, PREVIEW_LIMIT);
	const rows = visibleRecords.map(previewRow).join('');
	...
	return `
		<div class="wizard-content">
			...
			<div class="import-preview-summary" aria-label="Resumo da validação">
				<div><span>Total encontrado</span><strong>${draft.records.length}</strong></div>
				<div><span>Válidos</span><strong class="text-success">${valid}</strong></div>
				<div><span>Duplicatas</span><strong class="text-warning">${duplicates}</strong></div>
				<div><span>Inválidos</span><strong class="text-danger">${invalid}</strong></div>
			</div>
			${invalid ? `<div class="inline-notice inline-notice--danger"><strong>${pluralize(invalid, 'registro')} inválido(s) não será(ão) importado(s).</strong> Use “Voltar e corrigir” para ajustar o conteúdo.</div>` : '<div class="inline-notice inline-notice--success">✓ Nenhum registro inválido encontrado.</div>'}
			<div class="preview-table-wrap"><table class="preview-table"><thead><tr><th>Linha</th><th>Frente</th><th>Verso</th><th>Tags</th><th>Status</th><th>Detalhes</th></tr></thead><tbody>${rows}</tbody></table></div>
			<div class="import-preview-count">Exibindo ${visibleRecords.length} de ${draft.records.length} registro(s).${draft.records.length > PREVIEW_LIMIT ? ` A prévia foi limitada aos primeiros ${PREVIEW_LIMIT}; todos serão processados na importação.` : ''}</div>
...
function renderStepFour() {
	const result = draft.result ?? {processed: 0, imported: 0, invalid: 0, skipped: 0};
	...
	return `
		<div class="wizard-content empty-state">
			<div class="empty-state__content">
				...
				<p>${result.processed} registro(s) foram processados. Os cards aceitos foram enviados para <strong>${escapeHTML(deck?.name ?? 'o baralho escolhido')}</strong>.</p>
				<div class="metrics-grid" style="width:100%">
					<article class="metric-card"><div class="metric-card__top"><span>Processados</span><span>≡</span></div><strong>${result.processed}</strong><small>registros analisados</small></article>
					...
					<article class="metric-card"><div class="metric-card__top"><span>Inválidos</span><span>×</span></div><strong class="text-danger">${result.invalid}</strong><small>registros rejeitados</small></article>
				</div>
