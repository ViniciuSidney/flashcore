...
import {getActiveSession, getCurrentCard, getGradeIntervals, getLatestSession, isAnswerRevealed, sessionPreservesSchedule} from './review.service.js';
	...
	return `
		<div class="page-stack">
			<header class="page-intro">
				<div class="page-intro__copy"><span class="eyebrow">Estudo ativo</span><h2>Escolha como revisar</h2><p>Use a revisão programada para seguir os intervalos ou abra um baralho em Estudo livre sem alterar o cronograma.</p></div>
			...
			<section class="panel">
				<header class="panel-header"><div class="panel-header__copy"><span class="eyebrow">Por baralho</span><h2>Sessões direcionadas</h2><p>Escolha entre os cards pendentes ou um Estudo livre com todos os cards do baralho.</p></div></header>
...
function renderActiveReview(session) {
	...
	const gradeButton = (grade, className) => `
		<button class="grade-button ${className}" type="button" data-action="grade-card" data-grade="${grade}">
			...
			${preserveSchedule ? '<small>sem reagendar</small>' : showIntervals ? `<small>${intervals[grade].label}</small>` : ''}
		</button>
	`;
